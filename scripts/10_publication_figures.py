...
def figure2():
    ...
    for xi, v in zip(x + w/2, b3):
        axB.text(xi, v + 0.02, f"{v:.2f}", ha="center", fontsize=6.2, color=INK)
    axB.set_xticks(x); axB.set_xticklabels(CANON, rotation=20, ha="right")
    ...
    axD.text(0.02, overall_r + 0.02, f"overall r = {overall_r:.3f}",
             transform=axD.get_yaxis_transform(), fontsize=7, color=INK)
    axD.axhline(0, color=INK, lw=0.8)
    axD.set_xticks(np.arange(len(CANON))); axD.set_xticklabels(CANON, rotation=20, ha="right")
    axD.set_ylim(-0.05, 0.75); axD.set_ylabel("Pearson r (std vs |error|)")
    axD.set_title("Uncertainty tracks error", fontsize=9)
...
def figure3():
    ...
    axD.set_title("Conformal expansion of uncertainty", fontsize=9)
...
def figure5():
    ...
    axA1.text(1.35, 1.24, "Accuracy under reference reduction", transform=axA1.transAxes,
    ...
    axC.set_ylim(0, 0.75); axC.set_ylabel("Uncertainty–error correlation (Pearson r)")
    axC.set_title("Reliability signal weakens", fontsize=9)
